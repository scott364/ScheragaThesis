    ...
    def solve_inverse_kinematics(self, target_position, target_orientation=None, target_in_local_coords=False):
        ...
        if target_orientation is None:
            ik_solution = p.calculateInverseKinematics(
                self._simulator_id, self._end_effector_link_index, target_position, maxNumIterations=120)#120 default
        else:
            if len(target_orientation) == 3:
                target_orientation = p.getQuaternionFromEuler(
                    target_orientation)
            ik_solution = p.calculateInverseKinematics(
                self._simulator_id, self._end_effector_link_index, target_position, targetOrientation=target_orientation, maxNumIterations=120)#120 default
...
class SawyerMOD(Manipulator):
    ...
    def __init__(self, robot_name, position, orientation=[0, 0, 0, 1], fixed_base=0, publish_full_state=False):
        ...
        super().__init__(robot_name, 
                         ASSETS_PATH +'sawyer_description/urdf/sawyer_static_blockcombine2.urdf', position, orientation, fixed_base)
