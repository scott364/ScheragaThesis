...
dayTimeStamp = lambda: datetime.datetime.now().strftime(
    "%Y-%m-%d"
)  # http://stackoverflow.com/a/5215012/893511
...
nowTimeStamp = lambda: datetime.now().strftime(
    "%Y-%m-%d_%H-%M-%S"
)  # http://stackoverflow.com/a/5215012/893511
""" Return a formatted timestamp string, useful for logging and debugging """

nowTimeStampFine = lambda: datetime.datetime.now().strftime(
    "%Y-%m-%d_%H-%M-%S-%f"
)  # http://stackoverflow.com/a/5215012/893511
""" Return a formatted timestamp string, useful for logging and debugging """
        ...
        try:
            ...
            if Nverb > 0 and i % Nverb == 0:
                if HidePrintout == 0:
                    print("\n--------- Tick {0} ---------\n".format(i))
                    print(
                        "Root node, Name:", rootNode.name, ", Status:", rootNode.status
                    )
                    print("\n")
                if treeUpdate:
                    print(
...
class Finger_Width_COND(py_trees.behaviour.Behaviour):
    ...
    def update(self):
        ...
        if width >= (self.partWidth * (1.0 - self.margin)):
            ...
            if self._DEBUG:
                print(
                    "Finger_Width_COND - FAILED with desired ",
                    self.partWidth,
                    "-vs- actual",
                    width,
                )
...
class Move_Arm(py_trees.behaviour.Behaviour):  # BASIC
    ...
    def initialise(self):
        ...
        self.ctrl.arm.move_speed(
            self.pose, self.mode, self.speed, self.accel, 0, "dummy", self.blocking
        )
        time.sleep(0.1)
...
class Move_Arm_Relative(py_trees.behaviour.Behaviour):  # BASIC
    ...
    def __init__(
        ...
        if type(stop_cond) != type(None):
            self.cond_name = "condition"  # str( stop_cond.__name__ )[:]
            self.usng_cond = True
    ...
    def initialise(self):
        ...
        self.pose = rm.poses.translate_pose(self.pose, self.trans, dir_pose=self.frame)
        self.pose = rm.poses.rotate_pose(self.pose, self.rottn, dir_pose=self.frame)

        if self._DEBUG:
            print(
                "Linear Diff B/n Poses:",
                np.subtract(
                    rm.poses.position_from_pose(self.pose),
                    rm.poses.position_from_pose(orig),
                ),
            )

        if self._DEBUG:
            print("Wrench BEFORE bias:", self.ctrl.ft.get_wrist_force())
        ...
        if self._DEBUG:
            print("Wrench AFTER bias:", self.ctrl.ft.get_wrist_force())

        # Move arm
        if self.usng_cond:
            if self._DEBUG:
                print("Using the stop condition:", self.cond_name)
            self.ctrl.arm.move_speed(
                self.pose,
                self.mode,
                self.speed,
            ...
            if self._DEBUG:
                print("Moving without a force condition")
            self.ctrl.arm.move_speed(
                self.pose, self.mode, self.speed, self.accel, 0, _ALWAYS_FALSE, True
            )
        time.sleep(0.1)
        if self._DEBUG:
        ...
        if int(self.ctrl.arm.get_status_bits(), 2) == 1:
            ...
            if self.usng_cond:
                if self.cond_sccs:
                    if self.cond_met:
                        if self._DEBUG:
                            print("Ended FT move with condition MET --> SUCCESS")
                        return py_trees.common.Status.SUCCESS
                    else:
                        if self._DEBUG:
                            print("Ended FT move with condition UNMET --> FAILURE")
                        return py_trees.common.Status.FAILURE
                else:
                    if self.cond_met:
                        if self._DEBUG:
                            print("Ended FT move with condition MET --> FAILURE")
                        return py_trees.common.Status.FAILURE
                    else:
                        if self._DEBUG:
                            print("Ended FT move with condition UNMET --> SUCCESS")
                        return py_trees.common.Status.SUCCESS
            else:
                if (transErr <= _EPSILON_POSE_POSN) and (
                    orienErr <= _EPSILON_ORNT_EROR
                ):
                    if self._DEBUG:
                        print("Move_Arm_Relative: Ended at GOAL!")
                    return py_trees.common.Status.SUCCESS
                # C. Otherwise the goal has not been reached
                else:
                    if self._DEBUG:
                        print("Move_Arm_Relative: Goal NOT REACHED!")
                    return py_trees.common.Status.FAILURE
        # 2. Otherwise the robot arm has not stopped
        else:
            return py_trees.common.Status.RUNNING
...
class Xform_BB_Pose(py_trees.behaviour.Behaviour):
    ...
    def update(self):
        ...
        if self.rotationFirst:
            xformPose = rm.poses.rotate_pose(
                pose=pose, rotation_vec=rotn, dir_pose=self.rottnFrame
            )
            xformPose = rm.poses.translate_pose(
                pose=xformPose, translation_vec=tran, dir_pose=self.transFrame
            )
        else:
            xformPose = rm.poses.translate_pose(
                pose=pose, translation_vec=tran, dir_pose=self.transFrame
            )
            xformPose = rm.poses.rotate_pose(
                pose=xformPose, rotation_vec=rotn, dir_pose=self.rottnFrame
            )
...
class Move_Rule_w_Stop_Cond(py_trees.behaviour.Behaviour):
    ...
    def update(self):
        ...
        builtins._GLOB_FT_FLAG = 0
        self.ctrl.arm.move_speed(
            self.pose, "j", speed, accel, 0, self.give_cond(), True
        )  # Must be BLOCKING!
...
class Set_Grip_Torque(py_trees.behaviour.Behaviour):  # BASIC
    ...
    def __init__(self, torqState=1.0, DANGER_MODE=0, ctrl=None, name="Set_Grip_Torque"):
        """
        Minimal one-time initialisation, offline only:
