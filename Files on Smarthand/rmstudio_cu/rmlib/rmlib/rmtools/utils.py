...
from rmlib.rmtools import poses
...
def vec_unit(vec):
    ...
    )  # NOTE: This will throw an error for zero-length 'vec'
...
def np_add(*args):
    ...
    if (
        len(args) > 2
    ):  # If there are more than 2 args, add the first arg to recur on remainder of args
...
def np_subtract(*args):
    ...
    if (
        len(args) > 2
    ):  # If there are more than 2 args, subtract the last arg from the preceeding remainder of args
        # Note the star operator is needed for recursive call, unpack to positional args
...
def vec_linspace(vec1, vec2, numPts):
    ...
    for value in np.linspace(0, span, num=numPts):  # For each value in the range
        ptsList.append(
            np.add(
                vec1,  # append the sum of the first point and
                np.multiply(
                    direction,  # a vector that is an additional space along the difference
                    value,
                ),
            )
        )
...
def intersect_pnt_2D(seg1, seg2):
    ...
    if den == 0:
        ...
        if (uA >= 0 and uA <= 1) and (uB >= 0 and uB <= 1):
            ...
            return [
                seg1[0][0] + uA * (seg1[1][0] - seg1[0][0]),
                seg1[0][1] + uA * (seg1[1][1] - seg1[0][1]),
            ]
        else:
...
def rotate_vectors(
    ...
    pose = poses.rotate_pose(
        poses.origin_pose(), rx=rx, ry=ry, rz=rz, k=k, theta=theta, frame=frame
    )
...
def rand_sample_list_wo_replace(origList, N):
    ...
    rtnLst = []
    indices = np.random.choice(list(range(len(origList))), N, replace=False)
    for i in indices:
...
class HeartRate:
    ...
    def __init__(self, Hz):
        ...
        self.period = 1.0 / Hz
        # Set the period as the inverse of the frequency , hearbeat will not exceed 'Hz' , but can be lower
        self.last = time.time()
...
class TimerQueue(Queue):
    ...
    def start(self):
        ...
        self.worker = TimerThread(
            ...
            updateHz=self.updateHz,
            stopToken=self.stopToken,
        )
        self.running = True
        self.worker.start()
