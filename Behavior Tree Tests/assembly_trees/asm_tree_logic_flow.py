...
class Toggle_Var( py_trees.behaviour.Behaviour ): # BASIC
    ...
    def __init__( self , keyString = "toggleVar_" , initVal = 1 , ctrl = None ):
        ...
        if self._DEBUG: print( "Created `Toggle_Var` with key:" , self.key )
...
class Negator_DECO( py_trees.decorators.Decorator ):
    ...
    def update(self):
        ...
        if self.decorated.status == py_trees.common.Status.SUCCESS:
            self.feedback_message = "Negate:" + (" [%s]" % self.decorated.feedback_message if self.decorated.feedback_message else "")
    ...
    def update( self ):
        ...
        if not self.stopped:
            ...
            if self.decorated.status == py_trees.common.Status.FAILURE:
                self.feedback_message = "Allowed " + str( self.count ) + " of " + str( self.limit )  \
