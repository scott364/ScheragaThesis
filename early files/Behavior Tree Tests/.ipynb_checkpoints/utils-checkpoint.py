...
def vec_unit( vec ):
    ...
    # NOTE: This function forces the output to numpy array
    vec = np.array( vec ).astype( 'float64' ) # Force to float to (hopefully) suppress div0 warnings for int vectors
    mag = np.linalg.norm( vec , 2 )
    return np.divide( vec , mag )  # NOTE: This will throw an error for zero-length 'vec'
...
# ____ End Thread _____________________________________________________________________________________________________