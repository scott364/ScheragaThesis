...
class MainEnvRL(gym.Env):
    ...
    def motionselector(self,action):
        ...
        if phase ==2: 
            anglechange=2
            orientation_deg=quat2rpy(self.orientation) 
            
            if action==0:  
                self.zcurrent=round(self.zcurrent-movedist,3) 
            if action==1:  
                self.zcurrent=round(self.zcurrent-movedist,3) 
                orientation_deg[1] = orientation_deg[1] + (anglechange * (np.pi/180))  
            if action==2:  
                self.zcurrent=round(self.zcurrent-movedist,3) 
                orientation_deg[1] = orientation_deg[1] - (anglechange * (np.pi/180))
            if action==3:   
                self.zcurrent=round(self.zcurrent-movedist,3) 
                orientation_deg[2] = orientation_deg[2] + (anglechange * (np.pi/180))
            if action==4:   
                self.zcurrent=round(self.zcurrent-movedist,3)      
                orientation_deg[2] = orientation_deg[2] - (anglechange * (np.pi/180))
                
        self.orientation=rpy2quat(orientation_deg)      
