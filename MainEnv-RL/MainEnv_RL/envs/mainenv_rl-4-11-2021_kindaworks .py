...
class MainEnvRL(gym.Env):
    ...
    def __init__(self, render=True,totalepisodes=100):
        ...
        self.observation_space = spaces.Box(np.array([-1, -1, -1]), 
                                            np.array([1, 1, 1])) # pitch, gyro, com.sp.
    ...
    def resetEnvironment(self):
        ...
        self.xstart=random.uniform(self.targetx-(.5*0.0127) , self.targetx+(.5*0.0127) ) # 0.0127 m=0.5in
        self.ystart=random.uniform(self.targety-(.5*0.0127) , self.targety+(.5*0.0127) )
        #print("start location",self.xstart,self.ystart)
        self.zstart=0.84#0.87#0.95
...
#         time.sleep(.4)
    ...
    def _compute_done(self):
        ...
        if self._envStepCounter >= stepsPerEpisode:    
            ...
            if self.episodecounter%50==0 or self.episodecounter==self.totalepisodes:
                ...
                plt.setp(self.ax1, xlim=(0, self.totalepisodes), ylim=(-10,0))
        ...
        return self.currentreward > -0.01 or self._envStepCounter >= stepsPerEpisode
